...
def test_namespace_parse_no_at() -> None:
    agent_name, namespace = bus._parse_namespace_address("agent-a")
    assert agent_name == "agent-a"
    assert namespace is None


def test_namespace_parse_with_at() -> None:
    agent_name, namespace = bus._parse_namespace_address("agent-a@my-project")
    assert agent_name == "agent-a"
    assert namespace == "my-project"


def test_namespace_parse_empty_agent() -> None:
    agent_name, namespace = bus._parse_namespace_address("@ns")
    assert agent_name == ""
    assert namespace == "ns"


def test_register_namespace_writes_message(temp_bus: Path) -> None:
    bus.register_namespace("my-project", "target-bus")
    msgs = bus.read_messages(topic_filter="namespace-routing")
    assert len(msgs) == 1
    assert msgs[0]["type"] == "namespace_rule"
    assert msgs[0]["payload"]["namespace"] == "my-project"
    assert msgs[0]["payload"]["target_slug"] == "target-bus"
    assert msgs[0]["payload"]["action"] == "register"


def test_unregister_namespace_writes_message(temp_bus: Path) -> None:
    bus.register_namespace("my-project", "target-bus")
    bus.unregister_namespace("my-project")
    msgs = bus.read_messages(topic_filter="namespace-routing")
    assert msgs[-1]["payload"]["action"] == "unregister"


def test_get_namespace_mappings_empty(temp_bus: Path) -> None:
    assert bus.get_namespace_mappings() == []


def test_get_namespace_mappings_active(temp_bus: Path) -> None:
    bus.register_namespace("project-a", "bus-a")
    bus.register_namespace("project-b", "bus-b")
    mappings = bus.get_namespace_mappings()
    assert len(mappings) == 2
    ns = {m["namespace"] for m in mappings}
    assert ns == {"project-a", "project-b"}


def test_get_namespace_mappings_unregistered_removed(temp_bus: Path) -> None:
    bus.register_namespace("project-a", "bus-a")
    bus.register_namespace("project-b", "bus-b")
    bus.unregister_namespace("project-a")
    mappings = bus.get_namespace_mappings()
    assert len(mappings) == 1
    assert mappings[0]["namespace"] == "project-b"


def test_resolve_namespace_found(temp_bus: Path) -> None:
    bus.register_namespace("my-project", "target-bus")
    result = bus.resolve_namespace("my-project")
    assert result == "target-bus"


def test_resolve_namespace_not_found(temp_bus: Path) -> None:
    result = bus.resolve_namespace("nonexistent")
    assert result is None


def test_resolve_namespace_after_unregister(temp_bus: Path) -> None:
    bus.register_namespace("my-project", "target-bus")
    bus.unregister_namespace("my-project")
    result = bus.resolve_namespace("my-project")
    assert result is None
